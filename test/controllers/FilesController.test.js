...
describe('+ FilesController', () => {
  ...
  const mockUser = {
    email: 'ali@email.com',
    password: 'ssasagayo_ssagayo.ali',
  };
  ...
  const mockFiles = [
    ...
    },
    {
      ...
      type: 'file',
      data: [
        '+ Chapter 20: The Country that Had a Hero...!!',
        '+ Chapter 24: Speed!!',
        '+ Chapter 51: Beauty is the Beast!!',
        '+ Chapter 53: Sakura Decision!!',
        '+ Chapter 55: No Holds Barred!!',
  ...
  describe('+ POST: /files', () => {
    ...
    it('+ Fails if type is available but unrecognized', function (done) {
      ...
      request.post('/files')
        .set('X-Token', token)
        .send({ name: 'title_s.txt', type: 'manga' })
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
            return done(requestErr);
  ...
  describe('+ PUT: /files/:id/publish', () => {
    it('+ Fails with no "X-Token" header field', function (done) {
      request.put('/files/201820249/publish')
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
  ...
  describe('+ PUT: /files/:id/unpublish', () => {
    it('+ Fails with no "X-Token" header field', function (done) {
      request.put('/files/201820249/unpublish')
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
  ...
  describe('+ GET: /files/:id/data', () => {
    it('+ Fails if the file does not exist', function (done) {
      request.get('/files/201820249/data')
        .expect(404)
        .end((err, res) => {
          if (err) {
            return done(err);
